...
| Security and hostile input | Valid types alone are insufficient for a project that can contain huge, overlapping, or internally inconsistent collections. | Reject non-monotonic frames, overlapping notes, stale cents metadata, out-of-range MIDI, and oversized collections before opening. | Fuzzing and malformed-file corpus testing are not yet in CI. |
