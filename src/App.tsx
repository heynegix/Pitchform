...
import { base64ToBytes, createPitchformProject, isPitchformProject, projectToJson, sha256Hex } from './lib/project';
...
async function analyzeInWorker(samples: Float32Array, sampleRate: number): Promise<{ frames: PitchFrame[]; notes: Note[] }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = (event: MessageEvent<{ frames?: PitchFrame[]; notes?: Note[]; error?: string }>) => {
      worker.terminate();
      if (event.data.error || !event.data.frames || !event.data.notes) reject(new Error(event.data.error ?? 'Analysis failed.'));
      else resolve({ frames: event.data.frames, notes: event.data.notes });
    };
    worker.onerror = () => {
      worker.terminate();
...
export default function App() {
  ...
  const correctedSamples = useMemo(
    () => source ? renderCorrectedSamples(source.samples, source.buffer.sampleRate, notes) : null,
    [notes, source],
  );
  ...
  const activeUrl = previewMode === 'original' ? originalUrl : correctedUrl;
  ...
  const loadAudio = useCallback(async (file: File) => {
    ...
    try {
      ...
      const analysis = await analyzeInWorker(samples, buffer.sampleRate);
      setSource({ file, buffer, samples, frames: analysis.frames, fingerprint });
  ...
  const openProject = useCallback(async (project: PitchformProject) => {
    ...
    const file = new File([fileBytes], project.sourceAudio.name.replace(/\.[^/.]+$/, '') + '.wav', { type: 'audio/wav', lastModified: project.sourceAudio.lastModified });
    const { buffer } = await decodeAudioFile(file);
    const samples = audioBufferToMono(buffer);
    const projectNotes = cloneNotes(project.edits.notes);
    setSource({ file, buffer, samples, frames: project.analysis.frames, fingerprint: project.sourceAudio.sha256 });
  ...
  const handleSaveProject = () => {
    ...
    downloadBlob(new Blob([projectToJson(project)], { type: 'application/json' }), `${source.file.name.replace(/\.[^/.]+$/, '')}.pitchform`);
    setStatus('Project saved offline');
  };

  const handleExport = () => {
    if (!source || !correctedSamples) return;
    downloadBlob(new Blob([encodeWav(correctedSamples, source.buffer.sampleRate)], { type: 'audio/wav' }), `${source.file.name.replace(/\.[^/.]+$/, '')}-pitchform.wav`);
  ...
  const onTimeUpdate = () => {
    ...
    if (loopEnabled && audio.currentTime >= loopRange.end) {
    ...
    <div className="app-shell" onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="quiet-button" onClick={handleSaveProject} disabled={!source}>Save project</button>
          <button className="primary-button" onClick={handleExport} disabled={!source || isBusy}>Export WAV</button>
        </div>
      </header>
      ...
      <main className="workspace">
          ...
          <section className="editor-panel">
            <div className="editor-toolbar">
              <div className="toolbar-group">
                ...
                <button className="icon-button" onClick={() => { if (audioRef.current) audioRef.current.currentTime = 0; }} aria-label="Stop">■</button>
              ...
              <div className="toolbar-group center-tools">
                ...
                <button className={loopEnabled ? 'toggle-button active' : 'toggle-button'} onClick={() => setLoopEnabled((enabled) => !enabled)}>Loop</button>
              </div>
              <div className="toolbar-group">
        ...
        <div className="status-line" role="status"><span className={isBusy ? 'status-dot busy' : 'status-dot'} />{status}{loopEnabled && durationSeconds > 0 ? ` · loop ${formatTime(loopRange.start)}–${formatTime(loopRange.end)}` : ''}</div>
      ...
      <audio ref={audioRef} onTimeUpdate={onTimeUpdate} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={() => setIsPlaying(false)} />
